...
class ClientObject
{
	...
	public function doMetadataUpdateFromXsl($metadataObjectId, $xslFile, $successMessage, $trialsExceededMessage, $numberOfTrials) {
		if($numberOfTrials > 3) {
			echo $trialsExceededMessage;
			return;
		}
		...
		try {
			$metadataPlugin = KalturaMetadataClientPlugin::get($this->client);
			$metadataPlugin->metadata->updateFromXSL($metadataObjectId, $xslFile);
			echo $successMessage;
			...
			$this->doMetadataUpdateFromXsl($metadataObjectId, $xslFile, $successMessage, $trialsExceededMessage, ++$numberOfTrials);
		}
	}
